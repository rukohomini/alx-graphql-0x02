...
const Home: React.FC = () => {
  ...
  const { loading, error, data, refetch } = useQuery(GET_EPISODES, { variables: { page } });
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-[#A3D5E0] to-[#F4F4F4] text-gray-800">
      ...
      <main className="flex-grow p-6">
        ...
        <div className="flex justify-between mt-6">
          ...
          <button onClick={() => setPage(prev => prev < info.pages ? prev + 1 : prev)} className="bg-[#45B69C] text-white font-semibold py-2 px-6 rounded-lg shadow-lg hover:bg-[#3D9B80] transition duration-200 transform hover:scale-105">
            Next
          </button>
        </div>
      </main>
